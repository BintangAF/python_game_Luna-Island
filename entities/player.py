...
class Player(pygame.sprite.Sprite, Character):
    ...
    def _input(self):
        ...
        self.sprinting = bool((keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]) and moving and self.energy > 1)
    ...
    def _update_run_sound(self):
        ...
        run_key_down = bool(keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT])
        should_play = bool(run_key_down and move_key_down and self.sprinting and self.direction.length() > 0 and self.energy > 0)
        if self.run_sound:
